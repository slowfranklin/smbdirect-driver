/*******************************************************************************
 * smbdirect driver for Samba
 *
 * (c) Richard Sharpe <rsharpe@samba.org>
 * (c) Ralph Boehme  <slow@samba.org>
 *
 ...
 */
static void smbd_free_buffers(struct connection_struct *conn)
{
        if (conn->recv_mr) {
 ...
 */
static void smbd_clean_connection(struct connection_struct *conn, bool disconnect)
{
 ...
 * clean up.
 */
static void start_send(struct connection_struct *conn)
{
        int res = 0;
 ...
 * with.
 */
static int handle_recv_completion(struct ib_wc *wc, struct connection_struct *conn)
{
        int res = 0;
 ...
 */
static void conn_cq_work(struct work_struct *work)
{
...
static void handle_completion_event(struct ib_cq *cq, void *ctx)
{
        ...
        queue_work(conn->smbd_device->smbd_wq, &conn->cq_work);
}
 ...
 */
static int handle_connect_request(struct rdma_cm_id *cm_id,
                                  struct smbd_device *smbd_device)
{
        ...
        conn->smbd_device->connection_count++;

        cm_id->context = conn; /* How we find it again */
        conn->state = SMBD_NEGOTIATE;

        mutex_lock(&conn->smbd_device->connection_list_mutex);
        list_add_tail(&conn->connect_ent, &conn->smbd_device->connection_list);
        mutex_unlock(&conn->smbd_device->connection_list_mutex);
 ...
 */
static void smbd_disconnect_work(struct work_struct *work)
{
        struct connection_struct *conn = container_of(work,
                                                      struct connection_struct,
                                                      disconnect_work);
...
static int handle_disconnect(struct rdma_cm_id *cma_id)
{
        int res = 0;
        ...
        if (conn) {
                ...
                queue_work(conn->smbd_device->smbd_wq, &conn->disconnect_work);
        } else {
 ...
 * Handle CMA events ...
 */
static int smbd_cma_handler(struct rdma_cm_id *cma_id,
...
int smbd_listen(struct smbd_device *smbd_dev)
{
	...
	printk(KERN_INFO "Starting to listen for SMB-Direct on port %d\n", SMB_DIRECT_PORT);
